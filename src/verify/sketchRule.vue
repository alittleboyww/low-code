...
<template>
    <div class="demo">
      <div class="top">
        ...
        <button class="mr10 font18" @click="changeThick">标尺厚度</button>
        <button class="mr10 font18" @click="changeRange">宽高变更</button>
        <button class="mr10 font18" @click="changeStart">改变起始位置</button>
        <span>禁止缩放</span>
  ...
  const panzoomOption = reactive({
    ...
    disableZoom: false,
    // contain: 'inside',
    handleStartEvent: (event: MouseEvent) => {
  ...
  const changeWindowScale = () => {
    ...
    windowScale.value = Number(num.toFixed(2)) * 1
  }
  const resetMethod = () => {
    if (sketchruleRef.value) {
  ...
  const changeScale = (e: Event) => {
    panzoomOption.disableZoom = (e.target as HTMLInputElement).checked
  }
  const changeMove = (e: Event) => {
    panzoomOption.disablePan = (e.target as HTMLInputElement).checked
  }
  const changeShadow = () => {
  ...
  const startY = ref(100);
  const changeStart = () => {
    startX.value = Math.floor(Math.random() * 100)
    startY.value = Math.floor(Math.random() * 100)
    console.log(startX.value, startY.value)
  }
  const changeRange = () => {
    rectWidth.value = Math.floor(Math.random() * 1920)
    rectHeight.value = Math.floor(Math.random() * 1080)
  }
